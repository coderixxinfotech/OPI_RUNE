...
var report_to_indexer = false;
var report_url =
  process.env.REPORT_URL || "https://api.opi.network/report_block";
...
const PROGRESS_LOG_FILE = ord_folder + network_folder + "progress_log.txt";

async function getLastProcessedLine() {
  if (fs.existsSync(PROGRESS_LOG_FILE)) {
    const lastLine = await fs.promises.readFile(PROGRESS_LOG_FILE, "utf8");
    return parseInt(lastLine, 10) || 0;
  }
  return 0;
}

async function updateProgressLog(lineNumber) {
  await fs.promises.writeFile(PROGRESS_LOG_FILE, lineNumber.toString(), "utf8");
}
...
async function main_index() {
  ...
  while (true) {
    ...
    if (
      !fs.existsSync(ord_folder + network_folder + "runes_output_blocks.txt")
    ) {
      console.error("runes_output_blocks.txt not found, creating");
      fs.writeFileSync(
        ord_folder + network_folder + "runes_output_blocks.txt",
        ""
      );
    }

    let ord_last_block_height_q = await db_pool.query(
      `SELECT coalesce(max(block_height), -1) as max_height from runes_block_hashes;`
    );
    let ord_last_block_height = ord_last_block_height_q.rows[0].max_height;
    if (ord_last_block_height < fast_index_below) {
      // first inscription
    ...
    let ord_index_cmd =
      ord_binary +
      " --no-index-inscriptions --index-runes" +
      network_argument +
      " --bitcoin-data-dir " +
      chain_folder +
      " --data-dir " +
    ...
    const fileStream = fs.createReadStream(
      ord_folder + network_folder + "runes_output.txt",
      { encoding: "UTF-8" }
    );
    ...
    let startBlock = (await getLastProcessedLine()) || 839000;
    ...
    for await (const line of rl) {
      ...
      if (lines.length === 0 && line.includes("block_start")) lines.push(line);
      else if (lines.length > 0) lines.push(line);
      if (!blocks.includes(block)) blocks.push(block);
      if (line.includes("block_end") && blocks.length === 200) {
        break;
      }
    }
    ...
    lines_index = lines_index.filter((a) => blocks.includes(a.split(";")[1]));

    console.log(lines_index.length, "------Lines index");
    console.log(lines_index);
    // throw Error("00");

    let current_height_q = await db_pool.query(
      `SELECT coalesce(max(block_height), -1) as max_height from runes_block_hashes;`
    );
    ...
    for (const l of lines_index) {
      ...
      if (parts[2].trim() == "new_block") {
        ...
        let blockhash_db_q = await db_pool.query(
          "select block_hash from runes_block_hashes where block_height = $1;",
          [block_height]
        );
        if (blockhash_db_q.rows[0].block_hash != blockhash) {
          ...
          await db_pool.query(
            `INSERT into runes_indexer_reorg_stats
    ...
    for (let i = 0; i < lenlines; i++) {
      ...
      if (parts[2] == "block_start") {
        ...
        if (last_start_block != null && block_height <= last_start_block) {
          ...
          console.error(
            "start with less or equal block_height in latter: " + l
          );
        ...
        console.log({ next_expected_start, l });
        ...
        if (block_height != last_start_block) {
          console.error(
            "block_end block_height != block_start block_height: " + l
          );
          process.exit(1);
        }
        last_end_idx = i;
    ...
    if (!next_expected_start) {
      ...
      await db_pool.query(
        `INSERT into runes_indexer_work_stats
          (ord_index_tm, all_tm)
          values ($1, $2);`,
        [ord_index_tm, all_tm]
      );
    ...
    let current_runes_events_id_q = await db_pool.query(
      `SELECT coalesce(max(id), -1) as maxid from runes_events;`
    );
    let current_runes_events_id =
      parseInt(current_runes_events_id_q.rows[0].maxid) + 1;
    ...
    for (const l of lines) {
      ...
      if (parts[2] == "block_start") continue;
        ...
        if (block_height > current_height) {
          ...
          current_outpoint_to_pkscript_wallet_map[outpoint] = [
            pkscript,
            wallet_addr,
          ];
          ...
          running_promises.push(
            execute_on_db(sql_query_outpoint_to_balances_insert, [
              outpoint,
              pkscript,
              wallet_addr,
              rune_ids,
        ...
        if (block_height > current_height) {
          ...
          delayed_queries.push([
            0,
            sql_query_outpoint_to_balances_remove,
            [block_height, outpoint],
          ]);
        }
        ...
        if (block_height > current_height) {
          ...
          for (let i = 0; i < delayed_queries.length; i++) {
            if (
              delayed_queries[i][0] == 1 &&
              delayed_queries[i][2][4] == rune_id
            ) {
          ...
          delayed_queries.push([
            1,
            sql_query_id_to_entry_update,
            [burned, mints, block_height, rune_id, block_height],
          ]);

          running_promises.push(
            execute_on_db(sql_query_id_to_entry_changes_insert, [
              rune_id,
              burned,
              mints,
              block_height,
        ...
        if (block_height > current_height) {
          ...
          running_promises.push(
            get_info_and_insert_event(
              sql_query_runes_events_insert,
              txid,
              outpoint,
              rune_id,
              amount,
              block_height,
              current_runes_events_id
            )
          );
        ...
        if (block_height > current_height) {
          ...
          running_promises.push(
            execute_on_db(sql_query_runes_events_insert, [
              current_runes_events_id,
              1,
              txid,
              null,
      ...
      await updateProgressLog(blocks[blocks.length - 1]);
    }
    await Promise.all(running_promises);
    running_promises = [];
    ...
    for (const l of lines_index) {
      ...
      let block_height = parseInt(parts[1]);
      if (block_height < first_rune_height) {
        continue;
      }
      let blockhash = parts[3].trim();
    ...
    for (const k of Object.keys(to_be_inserted_hashes)) {
      ...
      await db_pool.query(
        `INSERT into runes_block_hashes (block_height, block_hash, block_time) values ($1, $2, $3) ON CONFLICT (block_height) DO NOTHING;`,
        [block_height, blockhash, blocktime]
      );
    }
    ...
    await db_pool.query(
      ...
      [
        main_min_block_height,
        main_max_block_height,
        ord_sql_query_count,
        new_runes_count,
        updated_runes_count,
        new_balances_count,
        removed_balances_count,
        added_entry_history_count,
        added_event_count,
        ord_index_tm,
        ord_sql_tm,
        update_log_tm,
...
async function try_to_report_with_retries(to_send) {
  ...
  console.log(
    "Error while reporting hashes to metaprotocol indexer indexer, giving up."
  );
}

async function update_cumulative_block_hashes(
  until_height,
  to_be_inserted_hashes
) {
  console.log("Updating cumulative block hashes");

  let last_hash_height_q = await db_pool.query(
    `SELECT coalesce(max(block_height), -1) as max_height from runes_cumulative_event_hashes;`
  );
  let last_hash_height = last_hash_height_q.rows[0].max_height;
  if (last_hash_height < 0) {
    last_hash_height = first_rune_height - 1;
  }
  ...
  for (let height = last_hash_height + 1; height <= until_height; height++) {
    let events_q = await db_pool.query(
      `SELECT event_type, outpoint, pkscript, rune_id, amount, block_height from runes_events where block_height = $1 order by id asc;`,
      [height]
    );
    ...
    let block_event_hash = bitcoin.crypto
      .sha256(Buffer.from(block_events_string, "utf8"))
      .toString("hex");
    let cumulative_event_hash = null;
    let cumulative_event_hash_q = await db_pool.query(
      `SELECT cumulative_event_hash from runes_cumulative_event_hashes where block_height = $1;`,
      [height - 1]
    );
    if (cumulative_event_hash_q.rows.length > 0) {
      let temp_concat =
        cumulative_event_hash_q.rows[0].cumulative_event_hash +
        block_event_hash;
      cumulative_event_hash = bitcoin.crypto
        .sha256(Buffer.from(temp_concat, "utf8"))
        .toString("hex");
    } else {
      cumulative_event_hash = block_event_hash;
    }
    await db_pool.query(
      `INSERT into runes_cumulative_event_hashes (block_height, block_event_hash, cumulative_event_hash) values ($1, $2, $3);`,
      [height, block_event_hash, cumulative_event_hash]
    );
  }

  if (report_to_indexer) {
    for (
      let height = Math.max(last_hash_height + 1, until_height - 9);
      height <= until_height;
      height++
    ) {
      let event_hash_q = await db_pool.query(
        `select block_event_hash, cumulative_event_hash from runes_cumulative_event_hashes where block_height = $1;`,
        [height]
      );
      ...
      if (!block_hash) {
        let block_hash_q = await db_pool.query(
          `select block_hash from runes_block_hashes where block_height = $1;`,
          [height]
        );
...
async function get_info_and_insert_event(
  query,
  txid,
  outpoint,
  rune_id,
  ...
  if (current_outpoint_to_pkscript_wallet_map[outpoint]) {
    ...
    let outpoint_info_q = await db_pool.query(
      `SELECT pkscript, wallet_addr from runes_outpoint_to_balances where outpoint = $1;`,
      [outpoint]
    );
...
async function handle_reorg(block_height) {
  ...
  await db_pool.query(
    `DELETE from runes_outpoint_to_balances where block_height > $1;`,
  ...
  await db_pool.query(
    `DELETE from runes_id_to_entry_changes where block_height > $1;`,
    [last_correct_blockheight]
  );
  let res = await db_pool.query(
    `SELECT rune_id from runes_id_to_entry where last_updated_block_height > $1;`,
    [last_correct_blockheight]
  );
  for (const row of res.rows) {
    let res_inner = await db_pool.query(
      `SELECT burned, mints, block_height from runes_id_to_entry_changes WHERE rune_id = $1 AND block_height <= $2 ORDER BY block_height desc LIMIT 1;`,
      [row.rune_id, last_correct_blockheight]
    );
    if (res_inner.rows.length == 0) {
      await db_pool.query(
        `UPDATE runes_id_to_entry SET burned = $1, mints = $2, last_updated_block_height = genesis_height WHERE rune_id = $3;`,
        [0, 0, row.rune_id]
      );
    } else {
      ...
      await db_pool.query(
        `UPDATE runes_id_to_entry SET burned = $1, mints = $2, last_updated_block_height = $3 WHERE rune_id = $4;`,
        [burned, mints, block_height, row.rune_id]
      );
    }
  }

  await db_pool.query(
    `DELETE from runes_cumulative_event_hashes where block_height > $1;`,
    [last_correct_blockheight]
  );
  await db_pool.query(
    `DELETE from runes_block_hashes where block_height > $1;`,
    [last_correct_blockheight]
  );

  await db_pool.query(
    `SELECT setval('runes_id_to_entry_id_seq', max(id)) from runes_id_to_entry;`
  );
  await db_pool.query(
    `SELECT setval('runes_outpoint_to_balances_id_seq', max(id)) from runes_outpoint_to_balances;`
  );
  await db_pool.query(
    `SELECT setval('runes_cumulative_event_hashes_id_seq', max(id)) from runes_cumulative_event_hashes;`
  );
  await db_pool.query(
    `SELECT setval('runes_block_hashes_id_seq', max(id)) from runes_block_hashes;`
  );
}
...
async function check_db() {
  ...
  try {
    let db_version_q = await db_pool.query(
      `SELECT db_version from runes_indexer_version;`
    );
    let db_version = db_version_q.rows[0].db_version;
    if (db_version != DB_VERSION) {
      console.error(
        "db_version mismatch, db needs to be recreated from scratch, please run reset_init.py"
      );
      process.exit(1);
    }
  } catch (err) {
    console.error(err);
    console.error(
      "db_version not found, db needs to be recreated from scratch, please run reset_init.py"
    );
    process.exit(1);
  }
  ...
  if (res_q.rows.length == 0) {
    console.error(
      "runes_network_type not found, db needs to be recreated from scratch, please run reset_init.py"
    );
    process.exit(1);
  }
  ...
  if (network_type_db != network_type) {
    console.error(
      "network_type mismatch, db needs to be recreated from scratch, please run reset_init.py"
    );
    process.exit(1);
  }
  ...
  let current_runes_id_to_entry_height_q = await db_pool.query(
    `SELECT coalesce(max(last_updated_block_height), -1) as max_height from runes_id_to_entry;`
  );
  let current_runes_id_to_entry_height =
    current_runes_id_to_entry_height_q.rows[0].max_height;
  console.log(
    "current_runes_id_to_entry_height: " + current_runes_id_to_entry_height
  );
  ...
  let current_runes_id_to_entry_changes_height_q = await db_pool.query(
    `SELECT coalesce(max(block_height), -1) as max_height from runes_id_to_entry_changes;`
  );
  let current_runes_id_to_entry_changes_height =
    current_runes_id_to_entry_changes_height_q.rows[0].max_height;
  console.log(
    "current_runes_id_to_entry_changes_height: " +
      current_runes_id_to_entry_changes_height
  );
  ...
  let current_runes_outpoint_to_balances_height_q = await db_pool.query(
    `SELECT coalesce(max(block_height), -1) as max_height from runes_outpoint_to_balances;`
  );
  let current_runes_outpoint_to_balances_height =
    current_runes_outpoint_to_balances_height_q.rows[0].max_height;
  console.log(
    "current_runes_outpoint_to_balances_height: " +
      current_runes_outpoint_to_balances_height
  );
  ...
  if (network_type == "mainnet") {
    ...
    let seed_rune_q = await db_pool.query(
      `SELECT count(*) as count from runes_id_to_entry where rune_id = $1;`,
      [seed_rune_id]
    );
    ...
    if (seed_rune_count == 0) {
      ...
      await db_pool.query(
        `INSERT into runes_id_to_entry (rune_id, rune_block, burned, divisibility, etching,
                                                          terms_amount, terms_cap, terms_height_l, terms_height_h,
                                                          terms_offset_l, terms_offset_h,
                                                          mints, "number", premine, rune_name, spacers, symbol,
                                                          "timestamp", turbo, genesis_height, last_updated_block_height) values ($1, $2, $3, $4, $5,
